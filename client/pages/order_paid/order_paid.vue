<template>
	<view>
		...
		<uni-card v-else v-for="(item, i) in order" :title="item.name" extra="已支付" :key="i">
			<span class="text">类型：{{ item.type }}</span>
			<span style="color: #f00;float: right;">￥{{ item.price }}</span>
			<br>
			<span class="text">地址：{{ item.address }}</span>
			<br>
			<span style="color: grey; font-size: 30rpx;">预定时间：{{ item['start-time'] }}</span><br>
			<span style="color: grey; font-size: 30rpx;">结束时间：{{ item['end-time'] }}</span>
